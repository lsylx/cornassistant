...
class MainActivity : ComponentActivity() {
        ...
        tag?.let { t ->
            ...
            if (ndef != null) {
                try {
                    ...
                    val counterInfo = readNtagCounter(tag)
                    onTagReadCallback?.invoke(
                        NfcReadResult(
                            uidHex = uidHex,
                            vcard = sb.toString(),
                            nfcCounter = counterInfo?.counter,
                            tagType = counterInfo?.tagType
                        )
                    )
                } catch (_: Exception) {
                    // ignore
        ...
        return try {
            ...
            if (ndef != null) {
                ...
                return if (ndef.isWritable) {
                    ...
                    ndef.close()
                    val counterInfo = if (enableCounter) configureNtagCounter(tag) else null
                    NfcWriteResult.Success(
                        uidHex = uidHex,
                        counterConfigured = counterInfo?.enabled == true,
                        counterTagType = counterInfo?.tagType
                    )
                } else {
                    ndef.close()
    ...
    private fun configureNtagCounter(tag: Tag): NtagCounterInfo? {
        ...
        return try {
            ...
            if (typeName == null) {
                return NtagCounterInfo(tagType = mapTypeName(type), enabled = false, counter = null)
            }

            val configPage = 0x2A
            val configBlock = ultralight.readPages(configPage)
            if (configBlock.size < 4) {
                return NtagCounterInfo(tagType = typeName, enabled = false, counter = null)
            }
            val current = configBlock.copyOfRange(0, 4)
            val enabled = (current[0].toInt() and 0x04) != 0
            if (!enabled) {
                val updated = current.clone()
                updated[0] = (updated[0].toInt() or 0x04).toByte()
                ultralight.writePage(configPage, updated)
            }
            val counterValue = tryReadCounter(ultralight)
            NtagCounterInfo(tagType = typeName, enabled = true, counter = counterValue)
        } catch (_: Exception) {
            NtagCounterInfo(tagType = mapTypeName(ultralight.type), enabled = false, counter = null)
        } finally {
            try {
                ultralight.close()
            } catch (_: Exception) {
    ...
    private fun readNtagCounter(tag: Tag?): NtagCounterInfo? {
        if (tag == null) return null
        val ultralight = MifareUltralight.get(tag) ?: return null
        return try {
            ultralight.connect()
            val type = ultralight.type
            val typeName = mapTypeName(type)
            if (typeName != "NTAG213" && typeName != "NTAG215" && typeName != "NTAG216") {
                return NtagCounterInfo(tagType = typeName, enabled = false, counter = null)
            }
            val counterValue = tryReadCounter(ultralight)
            NtagCounterInfo(tagType = typeName, enabled = counterValue != null, counter = counterValue)
        } catch (_: Exception) {
            null
        } finally {
            try {
    ...
    private fun tryReadCounter(ultralight: MifareUltralight): Int? {
        return try {
            val response = ultralight.transceive(byteArrayOf(0x39.toByte()))
            if (response.size >= 3) {
    ...
    private fun mapTypeName(type: Int): String? {
        return when (type) {
            MifareUltralight.TYPE_NTAG213 -> "NTAG213"
            MifareUltralight.TYPE_NTAG215 -> "NTAG215"
            MifareUltralight.TYPE_NTAG216 -> "NTAG216"
...
sealed class NfcWriteResult {
    ...
    data class Success(
        val uidHex: String,
        val counterConfigured: Boolean = false,
        val counterTagType: String? = null
    ) : NfcWriteResult()
...
private data class NtagCounterInfo(
    val tagType: String?,
    val enabled: Boolean,
    val counter: Int?
)
